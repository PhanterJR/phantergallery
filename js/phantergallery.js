            ...
            for (var i = fileslength - 1; i >= 0; i--) {
                ...
                if (img_type=="image/png"||img_type=="image/bmp"||img_type=="image/gif"||img_type=="image/jpeg"){
                    if(img_name.length<50){
                        MainObj.setTitleButton(MainObj.config.titleButtonProcessing)
                        ...
                        reader.onloadend = function(){
                            ...
                            MainObj.phanterGalleryCutterObj=phanterGalleryCutterObj(base64data, MainObj)
                        ...
                        var message = MainObj.config.messages.nameOfFileLong
                        MainObj.showMessage(message);
                    };
                } else{
                    var message = MainObj.config.messages.FileCouldNotBeUploaded.replace("%(name_file)s", img_name);
                    throw message
                    MainObj.showMessage(message);
                }
            }
...
var phanterGalleryCutterObj = function(base64data, MainObj){
    ...
    img1.src=base64data
    img2.src=base64data
    img1.onerror = function(){console.log("imagem inválida")}
...
/*window.onresize = function(event) {
    var back = phanterQuery("#phantergallery_panel-cutter-container")
    console.log(back.)
};*/
